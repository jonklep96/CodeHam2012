...
class Window:
        ...
        while True:
            for event in pygame.event.get():
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    sub_width = self.grid_surface.get_width()
                    sub_height = self.grid_surface.get_height()
                    sub_x = int(self.width - sub_width) / 2
                    sub_y = int(self.height - sub_height) / 2
                    self.screen.blit(self.grid_surface, (sub_x, sub_y))
                    self.clear_window()
    ...
    def draw_grid(self, first_run):
        ...
        for x in range(0, self.width, self.cell_width):
            for y in range(0, self.height, self.cell_height):
                ...
                pygame.draw.rect(self.grid_surface, color.WHITE, rect, self.GRID_CELL_WIDTH)
    ...
    def check_click(self, pos):
        ...
        for item in self.grid:
            if self.rect_contain(item, pos):
                ...
                pygame.draw.rect(self.grid_surface, color.BLUE, self.sel_cells[0], self.GRID_CELL_WIDTH + 1)
                pygame.draw.rect(self.grid_surface, color.BLUE, self.sel_cells[1], self.GRID_CELL_WIDTH + 1)
                pygame.draw.rect(self.grid_surface, color.BLUE, self.sel_cells[2], self.GRID_CELL_WIDTH + 1)
                pygame.draw.rect(self.grid_surface, color.BLUE, self.sel_cells[3], self.GRID_CELL_WIDTH + 1)
                pygame.draw.rect(self.grid_surface, color.YELLOW, new_rect, self.GRID_CELL_WIDTH + 2)
                self.last_loc = item
            grid_index += 1
    ...
    def clear_window(self):
        ...
        pygame.draw.rect(self.screen, color.GRAY, (0, 0, self.width, self.height))
